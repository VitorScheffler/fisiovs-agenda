...
export default function HomePage() {
  const todayAppointments = [
    { time: "09:00", patient: "Maria Silva", type: "Avaliação" },
    { time: "10:00", patient: "João Pereira", type: "Pilates" },
    { time: "14:00", patient: "Ana Costa", type: "Retorno" },
    { time: "15:00", patient: "Carlos Souza", type: "Tratamento" },
  ];

  const pendingRequests = [
    { patient: "Maria Silva", type: "Novo agendamento" },
    { patient: "João Pereira", type: "Remarcação" },
  ];
  ...
  return (
    <div className="flex min-h-screen bg-[var(--color-paper)] relative">
      ...
      <div className="hidden lg:contents">
        <Sidebar active="home" userName="Vitória Scheffler" userRole="Fisioterapeuta" />
      </div>
      ...
      <main className="flex-1 min-w-0 px-4 py-5 sm:px-6 md:px-8 md:py-6">
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4 mb-6 sm:mb-8">
          ...
          <div className="rounded-[14px] border border-[var(--color-line)] bg-[var(--color-card)] p-4 sm:p-5 shadow-[0_1px_3px_rgba(0,0,0,0.04)]">
            <p className="text-[13px] text-[var(--color-ink-soft)] mb-2">Atendimentos marcados</p>
            <p className="text-[26px] sm:text-[30px] font-semibold text-[var(--color-ink)]">2</p>
          </div>

          {/* Pacientes ativos */}
          <div className="rounded-[14px] border border-[var(--color-line)] bg-[var(--color-card)] p-4 sm:p-5 shadow-[0_1px_3px_rgba(0,0,0,0.04)]">
            <p className="text-[13px] text-[var(--color-ink-soft)] mb-2">Pacientes ativos</p>
            <p className="text-[26px] sm:text-[30px] font-semibold text-[var(--color-ink)]">6</p>
          </div>

          {/* Solicitações pendentes (alert) */}
          <div className="rounded-[14px] border border-[var(--color-terracotta-100)] bg-[var(--color-card)] p-4 sm:p-5 shadow-[0_1px_3px_rgba(0,0,0,0.04)] relative overflow-hidden">
            ...
            <p className="text-[26px] sm:text-[30px] font-semibold text-[var(--color-terracotta-600)]">2</p>
          </div>

          {/* Próximo atendimento */}
          <div className="rounded-[14px] border border-[var(--color-line)] bg-[var(--color-card)] p-4 sm:p-5 shadow-[0_1px_3px_rgba(0,0,0,0.04)]">
            <p className="text-[13px] text-[var(--color-ink-soft)] mb-2">Próximo atendimento</p>
            <p className="text-[26px] sm:text-[30px] font-semibold text-[var(--color-pine-600)]">13/06 19:00</p>
          </div>
        </div>

        {/* Two-column layout: agenda + right sidebar */}
        <div className="flex flex-col lg:flex-row gap-5 lg:gap-6">
          ...
          <div className="flex-1 min-w-0 rounded-[14px] border border-[var(--color-line)] bg-[var(--color-card)] p-4 sm:p-5 md:p-6 shadow-[0_1px_3px_rgba(0,0,0,0.04)]">
            <div className="flex items-center justify-between mb-5">
              ...
              <a href="#" className="text-[13px] font-medium text-[var(--color-pine-600)] hover:text-[var(--color-pine-700)] transition-colors">
                Ver agenda completa
              </a>
            </div>
              ...
              {todayAppointments.map((appt, idx) => (
                <div key={idx} className="flex items-start gap-4">
                  ...
                  <div className="flex-1 min-w-0">
                    ...
                    <p className="text-[13px] text-[var(--color-ink-soft)] mt-0.5">{appt.type}</p>
                  </div>
          ...
          <div className="w-full lg:w-80 xl:w-96 flex flex-col gap-4 sm:gap-5">
            ...
            <div className="rounded-[14px] border border-[var(--color-line)] bg-[var(--color-card)] p-4 sm:p-5 shadow-[0_1px_3px_rgba(0,0,0,0.04)]">
              ...
              <div className="grid grid-cols-2 gap-3">
                <button className="rounded-[12px] bg-[var(--color-pine-50)] border border-[var(--color-pine-100)] p-4 text-left hover:bg-[var(--color-pine-100)] transition-colors group">
            ...
            <div className="rounded-[14px] border border-[var(--color-terracotta-100)] bg-[var(--color-card)] p-4 sm:p-5 shadow-[0_1px_3px_rgba(0,0,0,0.04)]">
                ...
                {pendingRequests.map((req, idx) => (
                  <div key={idx} className="flex items-center justify-between py-1">
                    <div className="min-w-0">
                      ...
                      <p className="text-[12px] text-[var(--color-ink-soft)] mt-0.5">{req.type}</p>
                    </div>
                    <span className={`inline-flex rounded-full px-2 py-0.5 text-[10px] font-medium ${
                      req.type === 'Novo agendamento' ? 'bg-blue-50 text-blue-700' :
                      req.type === 'Remarcação' ? 'bg-amber-50 text-amber-700' :
              ...
              <button className="w-full rounded-[10px] border border-[var(--color-line)] px-4 py-2 text-[13px] font-medium text-[var(--color-ink)] hover:bg-[var(--color-paper)] transition-colors">
                Ver todas
              </button>
            </div>
          </div>
